...
?>
                <span class="alert alert-danger" id="venuealertt">
                  <strong>Error!</strong> Booking cannot be updated before 24 hrs
                </span>
<?php
            }
            else
            {
                $sql = "UPDATE events
                        SET  event_date = '$date', start_time = '$stime', end_time = '$etime', event_type = '$eventtype', event_status = '$eventstatus'
                        WHERE user_id = '$id' ";
...
<?php
        ...
        {
            $sql="INSERT INTO events (user_id, event_date, start_time, end_time, event_type, event_status)
                    VALUES('$id','$date','$stime','$etime','$eventtype','$eventstatus')";
